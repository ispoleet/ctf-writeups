#!/usr/bin/env python2
# -------------------------------------------------------------------------------------------------
# Teaser CONFidence CTF 2019 - elementary (RE 128)
# -------------------------------------------------------------------------------------------------
import struct
import sys
import os
...
def instrument(flag):
    ...
    #os.system("echo '%s' | %s -t %s -- %s > /dev/null" % (flag, PIN_BIN, INSTR_SO, ELEMENTARY_BIN))
    os.system("cat flag | %s -t %s -- %s > /dev/null" % (PIN_BIN, INSTR_SO, ELEMENTARY_BIN))
